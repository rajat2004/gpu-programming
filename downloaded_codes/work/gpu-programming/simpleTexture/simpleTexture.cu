...
__global__ void transformKernelGlobalMemory(float *outputData,
                                int width,
                                int height,
                                float theta, float *texglobal)
{
    // calculate normalized texture coordinates
    unsigned int x = blockIdx.x*blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y*blockDim.y + threadIdx.y;
...
for (unsigned ii = 0; ii < 100; ++ii) {
    transformKernel<<<dimGrid, dimBlock, 0>>>(dData, width, height, angle);
    //transformKernelGlobalMemory<<<dimGrid, dimBlock, 0>>>(dData, width, height, angle, texglobal);
